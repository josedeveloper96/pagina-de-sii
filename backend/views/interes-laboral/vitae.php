...
<td>
   ...
   <?= Html::a('Imprimir pdf', ['pdf', 'id' => $model->inl_id], ['class' => 'btn btn-primary']) ?>
...
<div id="containerCV">
<link rel="stylesheet"  href="http://itreynosa.edu.mx/sii/frontend/web/css/cv.css">
<?php

$array=Estudiantes::getPersonalData($model->inl_no_de_control);
$array2=PerfilEgresado::getPersonalData($model->inl_no_de_control);
$array3=InfAcademica::getPersonalData($model->inl_no_de_control);
$array4=InformacionLaboral::getProfesionalData($model->inl_no_de_control);
$array5=RefProfesionales::getProfesionalData($model->inl_no_de_control);
...
echo "</span>";
echo "<br>Domicilio: ";
echo "Calle ".$array2[0]['per_egr_calle'].", #".$array2[0]['per_egr_no'].", Col. ".$array2[0]['per_egr_colonia'].", ".$array2[0]['mpio_nombre'].", ".$array2[0]['edo_nombre']."<br>"; 
...
if(!empty($array2)){
    if ($array2[0]['per_image_web_filename']!='') {
        echo '<img id="FotoP" src="http://itreynosa.edu.mx/sii/frontend/web/'. 'uploads/perfiles/'.$array2[0]['per_img_scr_fname'].'">';
      }
}
echo "</td>";
echo "</tr></table>";
